    ...
    public partial class Form1 : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (conexionBD != null && conexionBD.State == ConnectionState.Open)
            {
                try
                {
                    string query = @"SELECT descripcion_origen, cR_origen, cG_origen, cB_origen, descripcion_destino, cR_destino, cG_destino, cB_destino FROM textura";
                    ...
                    using (MySqlDataReader reader = comando.ExecuteReader())
                    {
                        if (reader.HasRows)
                        {
                            while (reader.Read())
                            {
                                string origen = reader.GetString("descripcion_origen");
                                string destino = reader.GetString("descripcion_destino");
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            if (conexionBD != null && conexionBD.State == ConnectionState.Open)
            {
                try
                {
                    ...
                    if (x != 0)
                        MessageBox.Show("Se han realizado cambios en la imagen en texturas 10x10 px\n" + cad);
                    else
                        MessageBox.Show("No hay coincidencias en la base de datos");
                    conexionBD.Close();
                }
